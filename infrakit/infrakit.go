...
var (
	lock    = sync.Mutex{}
	plugins = map[string]func() (instance.Plugin, error){}
	log     = logutil.New("module", "infrakit", "plugin", "aws")
)

// Register registers a plugin implementation under a known name
func Register(provider string, startPlugin func() (instance.Plugin, error)) {
	lock.Lock()
	defer lock.Unlock()
	plugins[provider] = startPlugin
}

// GetInstancePlugin returns an instance plugin
func GetInstancePlugin(provider string) (instance.Plugin, error) {
	start, has := plugins[provider]
	if !has {
		return nil, fmt.Errorf("instance plugin not found: %s", provider)
	}
	return start()
}
...
func TemplateProcessor() (*pflag.FlagSet, ToJSONFunc, FromJSONFunc, ProcessTemplateFunc) {
				...
				return []template.Function{
					{
						...
						Func: func(prompt, ftype string, optional ...interface{}) (interface{}, error) {
							...
							// pipeline.
							if len(optional) > 0 && !cli.Missing(ftype, optional[len(optional)-1]) {
								return optional[len(optional)-1], nil
							}

							return cli.Prompt(os.Stdin, prompt, ftype, optional...)
						},
					},
				}
			})
